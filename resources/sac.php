...
function checkName() {
   ...
   <!-- **************  -->
	...
	if (currentName.value == '') {
		<!-- // currentName.value = 'test_' + Math.floor(Math.random() * 10000); -->
		<!-- ADD GEOLOCATION DATA FROM freegeoip.net -->
		<?php $remoteaddr = $_SERVER['REMOTE_ADDR']; 
		$xmllocdata = file_get_contents("http://freegeoip.net/xml/".$remoteaddr);
		$curuserlocinfo = simplexml_load_string($xmllocdata);
		?>
		currentName.value = '<?php echo $curuserlocinfo->City.$curuserlocinfo->RegionCode."User";?>' + Math.floor(Math.random() * 10000);
	}
};
